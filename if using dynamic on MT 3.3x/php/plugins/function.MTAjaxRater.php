...
function smarty_function_MTAjaxRater($args, &$ctx) {
	...
	list($vsumm) = $ctx->mt->db->load('ajaxrating_votesummary',$vote_args);
	if ($vsumm) {
		$avg_score = $vsumm['ajaxrating_votesummary_avg_score'];
		$vote_count = $vsumm['ajaxrating_votesummary_vote_count'];
		$total_score = $vsumm['ajaxrating_votesummary_total_score'];
	}
	if ($rater_type == 'star') {
